FundVoteView = {
  ...
  rednerOngoingExpVote: function(data) {
    ...
    var accnts = "";
    var HeaderBoxP1 = "<br/><div class='jumbotron' style='padding-top: 0.5em; padding-bottom: 0.5em;'>";
    var HeaderBoxP2 = "<h2 class='text-center'>Ongoing Ballots</h2></div>";

    accnts += HeaderBoxP1 + HeaderBoxP2;
    var set = false;

    for(var x=0; x<data.length; x++) {
        if(!data[x].complete) {
          var heading = "<tr><th colspan=\"2\"><label id=\"block"+x+"\">Ballot For Transaction</label></th>";
          heading += "</tr></thead>";

          var locAccnt = "<tr "+activeColor+"><td>To Account</td><td><label id=\"accnt"+x+"\" value=\""+data[x].account+"\">"+data[x].account+"</label></td></tr>";
          var locReason = "<tr "+activeColor+"><td>Reason</td><td><label id=\"reson"+x+"\">"+data[x].reason+"</label></td></tr>";
          var forAmount = "<tr "+activeColor+"><td>For Amount</td><td><label id=\"forAmount"+x+"\">"+data[x].amount+"</label></td></tr>";
          ...
          var yesBtn = "<tr "+activeColor+"><td><button onclick=\"FundVoteController.voteYes("+x+");\">Vote Yes</button></td>";
          var noBtn = "<td><button onclick=\"FundVoteController.voteNo("+x+");\">Vote No</button></td></tr>";

          accnts += tableHead + heading + locAccnt + locReason + forAmount + yesCount + noCount + yesBtn + noBtn + "</table><br/>";
        }
    }

    if(!set) {
        accnts += "<label>No Ongoing Ballots at this time.</label>";
    }
